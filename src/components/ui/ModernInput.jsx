...
const ModernInput = forwardRef(({
    ...
    // Only show error if field is touched or has a specific error
    const shouldShowError = !!error && (touched || error.length > 0);
    const isValid = !shouldShowError && props.value && props.value.toString().length > 0 && !props.disabled;
    ...
    return (
        <div className={`relative mb-5 group ${className}`}>
             <div className="relative">
                ...
                {label && (
                    <label
                        ...
                        className={`
                            ...
                            ${shouldShowError ? 'text-red-400' : focused ? 'text-gas' : 'text-gray-500'}
                ...
                {!multiline && (
                     <div className={`absolute right-4 top-1/2 -translate-y-1/2 flex items-center ${shouldShowError ? 'pointer-events-auto cursor-pointer' : 'pointer-events-none'}`}>
            ...
            {/* Error Text */}
            <AnimatePresence>
                {typeof error === 'string' && error.length > 0 && shouldShowError && (
                    <motion.div
