...
const App = ({ path, context }) => {
    ...
    useEffect(() => {
        ...
        // Added new URL slugs to valid list
        const validSections = [
            'start',
            'fluessiggastank-kaufen', // replaces 'tanks'
            'fluessiggas-bestellen', // replaces 'gas'
            'rechner',
            'gewerbe',
            'wissen',
            'ueber-uns',
        ...
        const isTankRoute = activeSection.startsWith('fluessiggastank-kaufen/'); // Updated prefix
    ...
    const renderSection = () => {
        // Check for dynamic routes - UPDATED SLUG
        ...
        if (!validSections.includes(activeSection)) {
            ...
            if (legacyTarget) {
                 const cleanTarget = legacyTarget.replace(/^\//, '');
                 // Updated prefixes for SSR redirect check
        ...
        switch(activeSection) {
            ...
            case 'fluessiggastank-kaufen': return <><TankSection openWizard={openWizard} setActiveSection={changeSection} isPageTitle={true} tankFilter={tankFilter} onFilterChange={setTankFilter} /><ContactSection /></>;
            case 'fluessiggas-bestellen': return <><GasOrderSection onCheckAvailability={handleGasCheckAvailability} setActiveSection={changeSection} /><FAQ /><ContactSection /></>;
